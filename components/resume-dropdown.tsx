  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="hidden md:inline-flex items-center gap-1.5 h-9 px-3 rounded-xl text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100/90 dark:bg-slate-800/70 border border-slate-200/60 dark:border-slate-700/60 hover:text-slate-900 dark:hover:text-white transition-colors whitespace-nowrap"
        >
      ...
      <div className="flex flex-col gap-1">
        {resumeOptions.map((resume) => (
          <a
            ...
            download
            className="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-slate-900/60"
          >
            {t(resume.labelKey)}
